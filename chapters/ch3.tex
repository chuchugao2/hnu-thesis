...
接着详细描述了CSM-TopK的基础框架以及密度剪枝策略的基线方案，并分析基于密度的剪枝的技术挑战以及本文的核心思想。
\section{引言}
...
基于此，本章提出并研究了一种基线方案，适用于基于TopK密度剪枝的连续子图匹配问题（CSM-TopK）。
...
在离线预处理阶段，我们首先为每条查询边 $(u_1, u_2) \in E_Q$ 构建一个匹配顺序。初始时，我们选择 $u_1$ 和 $u_2$ 作为起始节点（参见算法 \ref{alg:baseline:framework} 中的第 \ref{code:build-matching-order} 行），
...
例如，在图 \ref{fig:csm-topk}b 中，匹配顺序为 ${u_1, u_2, u_3, u_4}$，则：$LN(u_3) = \{u_1, u_2\}$，表示$u_3$的左邻居是$u_1$和$u_2$。这一结构对于后续的匹配扩展至关重要，因为它决定了如何从当前部分匹配出发，寻找可能的匹配扩展。
...
需要注意的是，本文的重点并不在于加速CSM解法，因此我们没有深入讨论一些基于拓扑的优化策略，如匹配顺序选择或拓扑剪枝等内容。
...
为应对这些挑战，我们提出了核心思想：构建压缩图上的全局MWstar和局部MWstar索引结构，这些索引结构帮助构建了紧密的密度上界函数，有效支持基于密度的剪枝策略，
并通过压缩图进一步提升了性能。
...
度剪枝不仅能减少计算量，还能提高搜索效率，尤其是在处理大规模数据时尤为重要。
...
在形成更新后的图$G_t$之后，我们将 $\mathcal{G}{t-1}$ 相应地转换为 $\mathcal{G}_t$。
