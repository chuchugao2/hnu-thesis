...
\begin{abstract}
	...
	针对上述问题，本文提出了一个新的研究问题——CSM-TopK，即在动态加权图中计算查询图的前k个密度最高的匹配结果，并证明该问题为NP难问题。本文的主要工作如下：
	\begin{enumerate}[label=(\arabic*)]  
	\item 为了有效解决CSM-TopK问题，我们首先定义了一种星形结构子图，并通过查询图匹配序列中节点的一阶邻居关系来导出该星形子图。
	\item 在星形结构子图的基础上，我们设计了两种轻量级索引，分别为全局MWstar索引和局部MWstar索引。具体而言，全局MWstar维护每个特定星形结构子图的最大权重，利用该最大权重，我们可以提前计算匹配子图的最大密度上限，从而进行剪枝，减少搜索空间，提高子图匹配的效率。
	...
	通过在五个真实世界数据集上的广泛实验，结果表明，结合图压缩技术和MWstar索引的最终方法表现最佳，在插入/删除的时间效率上，该方法相比现有方法至少提高了两个数量级；在索引的空间效率以及时间效率上也远超其他方法。实验结果验证了本文提出解决方案的有效性。
